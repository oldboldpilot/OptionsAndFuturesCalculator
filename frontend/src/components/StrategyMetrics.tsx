...
export function StrategyMetrics() {
  ...
  // Replaces the empty state when notReady is set, but never occludes a live result.
  // Unlike the tree pricer, commitTicket does NOT clear `result`. A user with a priced
  // position who then presses Add without a strike must keep seeing their position;
  // the prompt must not blank it.
  if (notReady) {
    return shell(
