...
FAST_MATH=$(grep -rnE "\-f""fast-math" "${REPO_ROOT}" 2>/dev/null | grep -v "cpp_details.txt" | grep -v "code_policy_check.sh" | grep -v "yaml" | grep -v "PRD_" | grep -v "backend/sensen" || true)
if [[ -n "$FAST_MATH" ]]; then
