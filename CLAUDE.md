...
**IT IS DEPLOYED AND VERIFIED LIVE, as of deployment `9e842180` on 2026-09-23.**
The paragraph above described a change that was built, tested and pushed but
NOT shipped for most of a day. Against the production ingress:

```
POST /sensen.finance.Finance/ComputePayment {"rate":"0.005625","periods":360,"present_value":"495000"}
-> {"value":"-3210.56057801266526493048779502036505463691"}     38 places
```

Cutover was confirmed the way this file requires — **4 `model is LOADED` lines**
(2 replicas x 2 assistants), 2 `SIMD: runtime tier avx512` lines, **0 `[WARN]`
and 0 `[ERROR]`**, all timestamped after the upload — not by a green
healthcheck, which passes before a throw.

**THE ONE THING THAT BROKE WAS A CONSUMER TEST PINNING THE SCALE, and
`docs/FINANCE_API.md` had told callers not to.** That document says in as many
words: *"Parse these as decimal strings of UNSPECIFIED length … and do not pin
the count."* `mortgage-nest-egg`'s `closing-costs-contract.test.ts` asserted
`/^-?\d+\.\d{18}$/` against every money field, so the deploy turned it red —
a test failing because the engine became MORE accurate, which is the wrong
direction to fail in.

It was caught by a **pre-deploy baseline**, not by reading: the same suite was
run against production's old engine first (20/20), so the single post-deploy
failure was attributable rather than ambiguous. Run the consumer's contract
suite before shipping a wire change; a bare pass afterwards cannot tell you
whether it ever passed.

The replacement asserts the property instead of the literal, and is **stronger
than what it replaced**: money is a STRING (never a JSON number, the reason the
rule exists), precision may grow but never silently shrink, and **every money
field shares ONE scale** — which the per-field literal could not see at all. It
is mutation-checked in both directions and confirmed the live engine emits a
uniform 38 places across all 17 money fields.

**THE CONTRACT HAS FOUR COPIES ACROSS TWO REPOSITORIES, and three of them still
said 18.** `backend/proto/finance.proto` is authoritative;
`clients/mortgagefv/proto/finance.proto` is a byte-copy handed to the consumer;
`mortgage-nest-egg` vendors its own copy of both. The authoritative file and
both vendored copies still described money as *"BigDecimal's own
18-decimal-place string"* six days after the engine began emitting 38 — so they
agreed with each other and all disagreed with the deployed engine. This is the
`ALLOWED_OPERATIONS` lesson again: a copy in another repository has no
mechanism here that could keep it honest.

`scripts/check_vendored_protos.sh` now gates it, wired into the `policy` job of
`.github/workflows/ci.yml`. **It asserts BYTE-IDENTITY against
`backend/proto/`, deliberately rather than recording a checksum.** The consuming
repository already gates its copy with a recorded sha256, and that manifest went
stale for a month — it named `62baf6ec` for a file that had become `68199ac9`,
so `sha256sum -c` reported the CORRECT contract as corrupt and the natural
repair was to restore the stale one. A recorded value must be regenerated by
hand and the hand forgets; identity against the source of truth is DERIVED, so
there is nothing to forget. Same reasoning as `pending_enqueues_in_log()`
replacing a cached counter. It carries a positive control — comparing zero files
is a FAILURE, not a pass — and all three arms are mutation-checked.

**The backend suite is NOT run in CI** (a C++23-modules + sensen + gRPC build
does not fit a hosted runner), so this gate is the only automatic guard on the
